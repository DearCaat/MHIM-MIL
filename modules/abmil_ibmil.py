...
class Dattention_ori(nn.Module):
    def __init__(self,input_dim,n_classes,dropout,act,confounder_path,**kwargs):
        super(Dattention_ori,self).__init__()
        self.L = 512
        self.D = 128
        self.K = 1
        self.embedding = FCLayer(in_size=input_dim,dropout=dropout,act=act)
        self.attention = nn.Sequential(
            nn.Linear(self.L, self.D),
            nn.Tanh(),
            nn.Linear(self.D, self.K)
        )   
        self.head = nn.Linear(self.L*self.K,n_classes)
        ...
        if confounder_path: 
            ...
            self.head =  nn.Linear(self.L*self.K+conf_tensor_dim, n_classes)
